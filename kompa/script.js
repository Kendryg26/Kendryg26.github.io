...
function loadTasks() {
    ...
    tasks.filter(task => task.date === selectedDate).forEach(task => {
        ...
        if (task.estado === "nopuse") nopuse++;
    });

    document.getElementById("hechasCount").textContent = hechas;
    document.getElementById("nopuseCount").textContent = nopuse;
}
...
function displayTask(text, date, time, estado) {
    ...
    if (estado === "hecha") li.classList.add("completed");
    ...
    li.innerHTML = `
        ...
        <div>
            <select onchange="cambiarEstado(this)" data-text="${text}" data-time="${time}">
                ...
                <option value="nopuse" ${estado === "nopuse" ? "selected" : ""}>No la puse</option>
            </select>
